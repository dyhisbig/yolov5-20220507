...
class Focus(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.conv(#卷积标准化激活函数
            # 640, 640, 3 => 320, 320, 12通道数变为原来的4倍，宽高减半
            torch.cat(
                [
                    x[..., ::2, ::2],
                    x[..., 1::2, ::2],
...
class Bottleneck(nn.Module):
    # Standard bottleneck残差结构，#Bottleneck就是两次卷积标准化激活函数加残差边
...
class C3(nn.Module):
    # CSP Bottleneck with 3 convolutions，重复残差结构的堆叠
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        # Bottleneck就是两次卷积标准化激活函数加残差边，堆叠n次
...
class CSPDarknet(nn.Module):
    def __init__(self, base_channels, base_depth, phi, pretrained):
        ...
        self.dark2 = nn.Sequential(
            ...
            Conv(base_channels, base_channels * 2, 3, 2),#卷积标准化激活函数
...
summary(csp, input_shape=(3, 640, 640), batch_size=-1)
